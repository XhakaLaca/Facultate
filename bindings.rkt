...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let ((seg(get-line-segment)))
    (let ((p1 (get-start-point seg))
          (p2 (get-end-point seg)))
...
(define (distance p1 p2)
  (compute-length (λ()(cons p1 p2)) car cdr ))
...
(define (compute-f-with-step f a b step)
  (let loop ((min a ) (max b) (m_step step) (function f) (res '()))
       (if (> min max)
        res
        (loop (+ min m_step) max m_step function ( append res (list(function min)))))))
...
(define (+ x y)  (string->number(string-append (number->string x) (number->string y))))
...
(define (compute-perimeter points)
  (let loop ((pts points) (res 0))
             (if(null? pts)
             res
             (if (equal? 1 (length pts))
             (loop (cdr pts) (+ res (distance (first points) (first pts))))
             (loop (cdr pts) (+ res (distance (first pts) (second pts))))))))
